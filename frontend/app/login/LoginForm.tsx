...
export default function LoginForm() {
  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      router.push(redirect ?? (user.role === "admin" ? "/admin/dashboard" : "/author/dashboard"));
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-100 px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-semibold text-slate-900">BookLeaf Publishing</h1>
        ...
        <div className="mt-4 bg-white rounded-xl border border-slate-200 p-4">
          ...
          <div className="space-y-2">
           {[
  { label: "Admin", sub: "admin@bookleaf.in", email: "admin@bookleaf.in", pw: "Admin@123" },
  { label: "Priya Sharma", sub: "priya.sharma@email.com", email: "priya.sharma@email.com", pw: "Author@123" },
  { label: "Rohit Kapoor", sub: "rohit.kapoor@email.com", email: "rohit.kapoor@email.com", pw: "Author@123" },
  { label: "Vikram Joshi", sub: "vikram.joshi@email.com", email: "vikram.joshi@email.com", pw: "Author@123" },
].map((d) => (
  <button key={d.email} onClick={() => fillDemo(d.email, d.pw)}
    ...
    <span className="text-slate-400 ml-2 text-xs">{d.sub}</span>
  </button>
))}
          </div>
        </div>
